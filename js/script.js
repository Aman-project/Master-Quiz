...
async function loadQuizzes() {
    ...
    if (storedQuizzes) {
        ...
        // Load default quiz from questions.json
        try {
            ...
            const defaultQuiz = {
                id: 'default_1',
                name: 'Communication Skills (Default)',
                questions: data
            };

            appState.quizzes = [defaultQuiz];
...
function saveNewQuiz() {
    ...
    try {
        ...
        if (!questions[0].hasOwnProperty('question') || !questions[0].hasOwnProperty('options') || !questions[0].hasOwnProperty('answer')) {
            throw new Error('Questions must have "question", "options", and "answer" fields.');
        }
...
function renderQuizList() {
    ...
    if (appState.quizzes.length === 0) {
        elements.quizList.innerHTML = '<p style="text-align:center; margin-top: 2rem;">No quizzes found. Create one to get started!</p>';
        return;
    }
    ...
    appState.quizzes.forEach(quiz => {
        ...
        card.innerHTML = `
            <div class="quiz-info">
                <h3><i class="ri-file-text-line" style="margin-right: 8px; vertical-align: bottom;"></i> ${quiz.name}</h3>
                <p style="padding-left: 2rem;">${questionCount} Questions</p>
            </div>
            <div class="quiz-actions" style="display: flex; gap: 0.5rem;">
                <button class="action-btn btn-start" onclick="selectQuiz('${quiz.id}')">
                    <i class="ri-play-fill"></i> Select
                </button>
                ${quiz.id.startsWith('custom_') ? `
                <button class="action-btn btn-delete" onclick="deleteQuiz('${quiz.id}')">
...
window.selectQuiz = function (id) {
    ...
    elements.totalQuestionsCount.textContent = quiz.questions.length;
...
function loadQuestion() {
    ...
    question.options.forEach(optionText => {
        ...
        // Add a circle icon that changes when selected
        button.innerHTML = `
            <i class="ri-checkbox-blank-circle-line" style="margin-right: 12px; font-size: 1.2rem; color: var(--text-muted);"></i>
            <span>${optionText}</span>
        `;
        button.onclick = () => handleOptionSelect(button, optionText, question.answer);
...
function updateHeader() {
    ...
    elements.questionNumber.textContent = `Q ${current}/${total}`;
    elements.scoreDisplay.textContent = `Score: ${appState.score}`;
...
function finishQuiz() {
    ...
    const accuracyVal = Math.round((correct / total) * 100);
    ...
    elements.accuracy.textContent = `${accuracyVal}%`;

    let msg = '';
    if (accuracyVal === 100) msg = 'Perfect Score! You are a genius!';
    else if (accuracyVal >= 80) msg = 'Excellent work! Keep it up!';
    else if (accuracyVal >= 50) msg = 'Good job, but room for improvement.';
    else msg = 'Keep practicing, you will get there!';
